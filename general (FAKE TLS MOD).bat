...
--filter-udp=443 --hostlist="%LISTS%list-user.txt" --dpi-desync=fake --dpi-desync-repeats=8 --dpi-desync-fake-quic="%BIN%quic_initial_www_google_com.bin" --new ^
...
--filter-tcp=80 --hostlist="%LISTS%list-user.txt" --dpi-desync=fake,split2 --dpi-desync-autottl=3 --dpi-desync-fooling=md5sig --new ^
...
--filter-tcp=443 --hostlist="%LISTS%list-user.txt" --dpi-desync=fake --dpi-desync-ttl=4 --dpi-desync-fake-tls-mod=rnd,rndsni,padencap --new ^
