...
if /i "!CHOICE!"=="y" (
	...
	powershell -Command "(New-Object Net.WebClient).DownloadFile('%RELEASE_URL%/download/%NEW_VERSION%/zapret-discord-youtube-%NEW_VERSION%.rar', 'zapretupdate%NEW_VERSION%.rar')"
	if exist "zapretupdate%NEW_VERSION%.rar" (
		echo downloaded v%NEW_VERSION%
	)
	echo checking winrar
	if exist %WINRAR% (
		echo winrar found
		echo 'backup your list-general.txt/list-discord.txt if you added custom urls!'
		pause
		powershell -Command "c:\zapret\service_remove.bat"
		echo waiting for zapret to stop...
		timeout 10
		"%WINRAR%" x -y "zapretupdate%NEW_VERSION%.rar" "c:\zapret\"
		del "zapretupdate%NEW_VERSION%.rar"
		powershell -Command "c:\zapret\service_install.bat"
	)
	pause
)
