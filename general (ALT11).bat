...
set "FSTUN2=%BIN%stun2.bin"
set "FDISCORD=%BIN%ACTIVE_DISCORD_UDP.bin"
set "FGAME=%BIN%ACTIVE_GAME_UDP.bin"

:: Добавляем autottl для улучшения обхода (можно закомментировать, если не нужно)
set "AUTOTTL=--dpi-desync-autottl=3"

:: Формируем списки портов с учётом GameFilter (если переменные пустые – не добавляем)
...
--filter-tcp=443 --hostlist="%LISTS%list-google.txt" --ip-id=zero --dpi-desync=fake,multisplit --dpi-desync-split-seqovl=681 --dpi-desync-split-pos=1 --dpi-desync-fooling=ts --dpi-desync-repeats=8 --dpi-desync-split-seqovl-pattern="%FTLS_GOOGLE%" --dpi-desync-fake-tls="%FTLS_GOOGLE%" %AUTOTTL% --new ^
--filter-tcp=80,443 --hostlist="%LISTS%list-general.txt" --hostlist="%LISTS%list-general-user.txt" --hostlist-exclude="%LISTS%list-exclude.txt" --hostlist-exclude="%LISTS%list-exclude-user.txt" --ipset-exclude="%LISTS%ipset-exclude.txt" --ipset-exclude="%LISTS%ipset-exclude-user.txt" --dpi-desync=fake,multisplit --dpi-desync-split-seqovl=664 --dpi-desync-split-pos=1 --dpi-desync-fooling=ts --dpi-desync-repeats=8 --dpi-desync-split-seqovl-pattern="%FMAX%" --dpi-desync-fake-tls="%FSTUN2%" --dpi-desync-fake-tls="%FMAX%" --dpi-desync-fake-http="%FMAX%" %AUTOTTL% --new ^
