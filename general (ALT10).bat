...
:: Выносим пути к фейкам в переменные для удобства
...
:: Формируем списки портов с учётом GameFilter
...
--filter-tcp=2053,2083,2087,2096,8443 --hostlist-domains=discord.media --dpi-desync=fake --dpi-desync-repeats=6 --dpi-desync-fooling=ts --dpi-desync-fake-tls="%FTLS_GOOGLE%" --dpi-desync-fake-tls-mod=none %AUTOTTL% --new ^
--filter-tcp=443 --hostlist="%LISTS%list-google.txt" --ip-id=zero --dpi-desync=fake --dpi-desync-repeats=6 --dpi-desync-fooling=ts --dpi-desync-fake-tls="%FTLS_GOOGLE%" %AUTOTTL% --new ^
--filter-tcp=80,443 --hostlist="%LISTS%list-general.txt" --hostlist="%LISTS%list-general-user.txt" --hostlist-exclude="%LISTS%list-exclude.txt" --hostlist-exclude="%LISTS%list-exclude-user.txt" --ipset-exclude="%LISTS%ipset-exclude.txt" --ipset-exclude="%LISTS%ipset-exclude-user.txt" --dpi-desync=fake --dpi-desync-repeats=6 --dpi-desync-fooling=ts --dpi-desync-fake-tls="%FSTUN%" --dpi-desync-fake-tls="%FTLS_4PDA%" --dpi-desync-fake-http="%FMAX%" %AUTOTTL% --new ^
