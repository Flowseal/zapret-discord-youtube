...
Если у тебя Windows 11, то обнови ipset (service.bat > Update IPSet List), включи Game Filter (Game Filter > UDP Only или UDP+TCP), и настрой Secure DNS прямо в настройках ОС Windows 11. Про настройку Secure DNS для Windows 11 написано в самом начале этого файла и есть ссылка на инструкцию.

Если у тебя Windows 10, то скорее всего Roblox у тебя работать не будет. Ты можешь попробовать настроить DNS серверы Google, или Cloudflare, или Quad9, - вместо автоматически используемых DNS серверов. Но это не гарантирует работу Roblox, т.к. DNS в этом случае не зашифрован и может быть перехвачен провайдером.
