...
> [!TIP]
> Решения выше по списку приоритетнее

### `winws` не запускается вручную

- Удалите службы `zapret` и `WinDivert` ( `service.bat` > `Remove Services`)
...
- `service.bat` > `Run Diagnostics`, исправьте проблемы, если имеются
...
### Не работают игры
- `service.bat` > `Update IPSet List` и включите `Game Filter`
...
https://github.com/bol-van/zapret-win-bundle/tree/master/windivert-hide

### WinDivert остается в службах после удаления

Запустите командную строку Windows от имени администратора и введите следующие команды:
```cmd
sc stop WinDivert
...
[zapret имеет лицензию MIT](https://github.com/bol-van/zapret/blob/master/docs/LICENSE.txt), как и эта сборка (см. [LICENSE.txt](./LICENSE.txt)).
