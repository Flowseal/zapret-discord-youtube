...
> [!IMPORTANT]
> Запрет НЕ ЯВЛЯЕТСЯ ВПН или ПРОКСИ, поэтому с регионально заблокированными ресурсами он ничего не сделает. Он работает исключительно с  DPI. А так же не будет работать как они. Если ресурса или его IP адреса нет в листах - он ничего с ним не сделает в попытках обхода DPI. Все совпадения по тому, что у вас что-то работает без внесения в списки - чисто случайны, в зависимости от настроек программы. Либо адресов IP set, которые выпадают на смежные с серверами обхода ютуба или дискорда (к примеру ака cloudflare). 

> [!CAUTION]
> IP set any должен быть использован только в тестировочных целях, лишь когда не работает нужный вам сайт при добавлении его в лист, но без добавления IP адресов сайта. В нормальном режиме программы он должен стоять либо на None, либо на Loaded. Если вы ставите IP set на any на постоянную основу, то вы принимаете все риски того, что он может вполне себе поломать кучи других ресурсов т.к. действовать начинает на все IP адреса. 
