...
- [**`service.bat`**](./service.bat) - установка в автозапуск и другие функции:
  ...
  **После переключения требуется перезапуск стратегии.**  
  ...
  - **`Auto-Update Check`** - Вкл/Выкл автоматическую проверку обновлений при запуске `general*.bat`, независимо от выбранного режима обновления
  ...
  - **`Check for Updates`** - ручная проверка обновлений в выбранном режиме, даже если автоматическая проверка выключена
  - **`Update mode`** (пункт **13**, номера 1-12 не изменены) - переключение `GitHub` / `Install`. По умолчанию `GitHub`: прежнее поведение с открытием страницы нового релиза. `Install`: скачивание и установка после подтверждения Y/N
...
- **`Update mode` -> `GitHub` используется по умолчанию**: отсутствие `utils\auto_update.enabled` означает прежнее поведение. При обнаружении более новой версии открывается её страница GitHub, без скачивания архива, требований к ZIP/SHA-256 и запроса UAC со стороны updater. Само меню `service.bat` по-прежнему запускается с правами администратора.
- **`Update mode` -> `Install`** создаёт `utils\auto_update.enabled`: после подтверждения Y/N updater скачивает и устанавливает релиз, проверяя SHA-256 архива перед заменой. Переключение обратно в `GitHub` удаляет маркер. Выбранный режим применяется и к ручному **`Check for Updates`**, и к проверке при запуске `general*.bat`; последняя выполняется только при включённом **`Auto-Update Check`** (`utils\check_updates.enabled`).
- Дальнейшее описание установки относится только к режиму **`Install`**.
- Обновление выполняется **поверх файлов текущей установки (in-place)**, без переименования или замены всей папки. Updater запускает временную копию себя, ждёт завершения проверяющего PowerShell и ещё 2 секунды, чтобы вызывающий BAT завершился через код 20.
- Останавливается только `bin\winws.exe` из обновляемой папки и служба `zapret`, если её `PathName` указывает именно на этот EXE. При чужой или неоднозначной службе обновление прерывается без её остановки. После установки или успешного отката запрашивается восстановление прежнего запуска; повторная проверка из BAT отключается через `NO_UPDATE_CHECK`.
- Сохраняются существующие `lists\ipset-exclude-user.txt`, `lists\list-general-user.txt`, `lists\list-exclude-user.txt`, `lists\ipset-all.txt` и его `.backup`, `utils\targets.txt`, `bin\ACTIVE_DISCORD_UDP.bin`, `bin\ACTIVE_GAME_UDP.bin`, `ipset_switched.flag`. Маркеры `utils\game_filter.enabled`, `utils\check_updates.enabled` и `utils\auto_update.enabled` сохраняются, включая их отсутствие. Остальные файлы, которых нет в релизе (например, дополнительные BAT), остаются на месте.
- Перед копированием рядом создаётся `.backup-ДАТА-ВРЕМЯ-ID` **только заменяемых существующих файлов**, а не полная копия установки. При ошибке копирования updater восстанавливает их и удаляет добавленные файлы; пустые созданные каталоги могут остаться. Если откат неполон, об этом сообщается явно и запуск не восстанавливается. Ошибка запуска после завершённого копирования не отменяет установленное обновление.
- Это не атомарная установка: выключение питания или принудительное завершение updater могут оставить частичное обновление. Не запускайте стратегии и не редактируйте файлы во время обновления. Антивирус или блокировки файлов могут помешать копированию и откату. Сохранённые backup и временные файлы можно удалить вручную после проверки работы; лог `%TEMP%\zapret-discord-youtube-update.log` записывается по возможности. Проверка доступности сайтов автоматически не выполняется.
- Версия берётся из константы `LOCAL_VERSION` в `service.bat`; для режима `Install` релиз обязан содержать соответствующий ZIP и SHA-256: поле `digest` ZIP-asset в GitHub API либо отдельный `.zip.sha256`. Если доступны оба значения, они должны совпадать. Без контрольной суммы установка запрещена. В режиме `GitHub` эти assets не нужны. SHA-256 проверяет целостность, но не является независимой подписью релиза.
- Устанавливаемый ZIP должен содержать `utils\update.ps1` и интеграцию обновлений в BAT. Архив без updater отклоняется до замены файлов. Источник обновлений — релизы `Flowseal/zapret-discord-youtube`; установка рассчитана на релизы, включающие эту возможность.
- `-NonInteractive` отключает диалоговые окна при `-Apply`. При проверке с `-Install` этот параметр запрещает установку вместо ожидания ввода Y/N; автоматического согласия он не даёт.
- Автоматическая установка поддерживается на Windows 10/11 с Windows PowerShell 5.1.

Для ручной полной переустановки:
...
На Windows 10/11 запустите из корня проекта:

```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File ".\utils\test update.ps1"
```

Тест создаёт минимальную временную установку с фиктивным EXE и локальный ZIP. Проверяет версию из `service.bat`, in-place backup, сохранение пользовательского списка и состояний маркеров, отказ при неверном SHA-256/версии/опасном ZIP-пути и отсутствии updater, восстановление ранее заменённых файлов и сообщение о неполном откате при блокировке файла. Проверка владельца службы тестируется на подставных данных; все вызовы установки используют `-SkipRuntimeControl`, рабочая установка и службы не изменяются. Временные файлы и результаты сохраняются по пути, который выводится в конце теста. Реальные UAC, handoff BAT, остановку и восстановление запуска нужно проверять отдельно на Windows.

Для проверки синтаксиса PowerShell, сравнения версий, кавычек аргументов Windows, чтения GitHub digest и проверки владельца службы на подставных данных без установки используйте `-StaticOnly` на Windows. Также проверяются режимы обновления на подставных ответах GitHub: открытие страницы без assets, отсутствие действий при равной/старой версии, отказ без контрольной суммы, отказ N и запрет интерактивного запроса с `-NonInteractive` в режиме Install. Браузер, сеть и UAC в этих тестах не запускаются.
