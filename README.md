...
> [!IMPORTANT]
> Все ии системные файлы в папке [`bin`](./bin) взяты из [zapret-win-bundle/zapret-winws](https://github.com/bol-van/zapret-win-bundle/tree/master/zapret-winws) и [zapret/releases](https://github.com/bol-van/zapret/releases). Вы можете это проверить с помощью хэшей/контрольных сумм.
>
> Проверяйте, что запускаете, используя сборки из интернета! А ещё не качайте по ссылкам из Яндекс Поиска! Яндекс Поиск часто предлагает вредоносные (вирусные) сборки этой программы, а настоящий репозиторий в Поиске скрыт.
