...
- [**`strategy_finder.bat`**](./strategy_finder.bat) - автоматический поиск рабочей стратегии

  Автоматически перебирает все стратегии и комбинации параметров, проверяя реальную доступность YouTube (включая видео) и Discord (включая CDN). Подробнее см. раздел [🔍 Strategy Finder](#-strategy-finder)
...
Утилита для автоматического поиска рабочей стратегии обхода. Перебирает все `general*.bat` и генерирует комбинации параметров, проверяя реальную доступность сервисов.

### Меню

```
[1] Quick Start - YouTube (video check)
[2] Quick Start - Discord (CDN + gateway)
[3] Quick Start - Both YouTube + Discord
[4] Manual Setup
[5] Test .bat strategies only
[6] Quick test single params
[7] Diagnostics
[0] Exit
```
...
<details>
<summary><b>[1] Quick Start - YouTube</b></summary>

**Что проверяет:**
- `youtube.com` — главная страница
- `i.ytimg.com` — CDN с превьюшками видео (если работает — видео тоже будет грузиться)

**Алгоритм:**
1. Останавливает текущий `winws.exe`
2. Проверяет YouTube БЕЗ zapret — вдруг уже работает
3. **Фаза 1**: перебирает все `general*.bat` файлы
   - Запускает каждый .bat → ждёт 7 сек → проверяет curl'ом
   - Если работает → **SUCCESS**, останавливается
4. **Фаза 2**: генерирует комбинации параметров (fake, multisplit, fooling, repeats...)
5. Рабочие сохраняет в `working_configs.txt`

</details>

<details>
<summary><b>[2] Quick Start - Discord</b></summary>

**Что проверяет:**
- `discord.com` — главная
- `cdn.discordapp.com` — картинки, аватарки, файлы

**Алгоритм:** тот же, только проверяет Discord вместо YouTube.

</details>

<details>
<summary><b>[3] Quick Start - Both</b></summary>

Проверяет **И YouTube И Discord**. Стратегия считается рабочей только если оба сервиса работают одновременно.

</details>

<details>
<summary><b>[4] Manual Setup</b></summary>

Ручная настройка всех параметров:
- Что проверять (YouTube / Discord / Both)
- Режим (только .bat / только генерация / всё)
- Уровень перебора (1-4)
- Остановиться после первой рабочей (Yes/No)

</details>

<details>
<summary><b>[5] Test .bat strategies only</b></summary>

Только Фаза 1 — быстрый перебор готовых `general*.bat` без генерации параметров.
Занимает 2-5 минут на все ~19 стратегий.

</details>

<details>
<summary><b>[6] Quick test single params</b></summary>

Тест одной конкретной комбинации параметров. Есть пресеты или ручной ввод.
Если работает — предлагает сохранить как .bat файл.

</details>

<details>
<summary><b>[7] Diagnostics</b></summary>

Проверяет:
- Права администратора
- Наличие curl, winws.exe, списков
- Текущую доступность YouTube и Discord
- Запущен ли winws.exe (и пробует убить)

</details>

### Уровни перебора

| Уровень | Комбинаций | Время | Когда использовать |
|---------|------------|-------|-------------------|
| 1 (Fast) | ~50 | 5-10 мин | Быстрая проверка |
| 2 (Medium) | ~150 | 20-40 мин | Рекомендуется |
| 3 (Deep) | ~400 | 1-2 часа | Если уровень 2 не помог |
| 4 (Full) | ~1000+ | 3+ часа | Полный перебор |

### Рекомендуемый порядок

1. **[7] Diagnostics** — убедиться что всё на месте
2. **[5] .bat only** — быстро прогнать готовые стратегии
3. Если ничего → **[1] или [3]** с уровнем 2
4. Если ничего → **[4] Manual** с уровнем 3-4
5. Нашли → **[6] Quick test** для тонкой настройки

### Результаты

Все рабочие конфигурации сохраняются в `working_configs.txt`
