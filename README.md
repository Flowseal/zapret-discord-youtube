...
*   **`WinDivert open error: access denied (код 5)`**
    ...
    *   **Решение:**
    *    1. Убедитесь, что в пути к папке с проектом **нет кириллицы (русских букв)**, а сам проект распакован на локальный диск, а не находится в облачном хранилище (например, OneDrive, iCloud или Яндекс.Диск).
