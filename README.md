...
- [**`service.bat`**](./service.bat) - подбор стратегий, установка в автозапуск и другие функции:
  - **`Autotune`** - автоматический подбор наилучшей рабочей стратегии обхода
  - **`Startup Health Check`** - проверка доступности при входе в систему (при сбое предложит запустить Autotune)
  - **`Check Status`** - проверка статуса обхода и активной стратегии (в режиме службы или батника)
...
- Запустите **`service.bat`** -> **`Autotune`** для автоматического подбора новой рабочей стратегии
...
- Далее используйте **`Autotune`** в `service.bat` для подбора стратегии или пробуйте запускать различные `general` скрипты вручную
