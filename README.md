...
- **`list-general-user.txt`** для доменов (поддомены автоматически учитываются). Также добавлен файл `list-general-user.example.txt` в качестве примера. Его можно переименовать в `list-general-user.txt` или не трогать
