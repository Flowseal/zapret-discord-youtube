...
- [**`service.bat`**](./service.bat) - установка в автозапуск и другие функции:
  ...
  **После переключения требуется перезапуск стратегии.**  
  ...
  - **`Update mode`** (пункт **11**) - переключение `GitHub` / `Install`. По умолчанию `GitHub`: прежнее поведение с открытием страницы нового релиза. `Install`: скачивание и установка после подтверждения Y/N
...
- После успешной установки используются параметры выбранной стратегии из нового BAT, включая сохранённую команду службы. При откате восстанавливаются прежние параметры. Если работающую стратегию невозможно однозначно определить или прочитать, обновление прерывается до её остановки: закройте стратегию и запустите нужный BAT для обновления.
