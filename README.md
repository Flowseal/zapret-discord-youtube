...
#### Работа не гарантирована. Если не помогло - используйте [Desktop версию](https://github.com/telegramdesktop/tdesktop/releases) с [локальным прокси](https://github.com/Flowseal/tg-ws-proxy) или любыми публичными.
...
*   **`WinDivert open error: access denied (код 5)`**
    ...
    *   **Решение:**
    *    1. Убедитесь, что в пути к папке с проектом **нет кириллицы (русских букв)**, а сам проект распакован на локальный диск, а не находится в облачном хранилище  **OneDrive**.
    *    2. Закройте окно. Нажмите на `.bat` файл правой кнопкой мыши и выберите **«Запуск от имени Администратора»**.
...
*   **`WinDivert open error: The handle is invalid (код 6)` или зависание на старте**
    ...
    *   **Решение:** Полностью закройте/удалите GoodbyeDPI, ByeDPI, Spoiler, Amnezia или сторонние VPN. Запустите `service.bat` от имени администратора, после чего выполните `Remove Services`, перезагрузите ПК и попробуйте снова.
...
*   **`CreateServiceA failed! Error code: 1073 (The specified service already exists)`**
    ...
    *   **Решение:** Запустите файл `service.bat` от имени администратора, после чего выполните `Remove Services`, чтобы полностью стереть старую службу, после чего повторите установку.
